...
/// screen means Pi is working. When no working or blocked signal matches,
/// Pi is idle.
fn classify_pi(screen_tail: &str) -> DetectedTerminalAgentStatus {
    ...
    let generic_status = classify_generic(screen_tail);
    if generic_status != DetectedTerminalAgentStatus::Unknown {
...
mod tests {
    ...
    fn classifies_pi_status() {
        ...
        assert_eq!(
            classify(
                "pi",
                "────────────────\n \n────────────────\n~/projects/Pentip (main)\n0.0%/1.0M (auto)  glm-5.2 • high",
                "",
            ),
            DetectedTerminalAgentStatus::Idle
        );
